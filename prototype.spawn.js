...
StructureSpawn.prototype.spawnCreepsIfNecessary = function() {
  ...
  let maxEnergy = 300;
  console.log(
    "Energy avail in " + room + " is: " + room.energyAvailable + "/" + maxEnergy
  );
  let name = undefined;

  // if no harvesters are left AND either no lorries are left
  ...
  if (name == undefined) {
    for (let role of listOfRoles) {
      ...
      else if (role == "attacker" && this.memory.attackRoom != undefined) {
        // try to spawn a claimer
        name = this.createAttacker(this.memory.attackRoom);
        // if that worked
        if (name != undefined && _.isString(name)) {
          // delete the claim order
      ...
      // if no claim, reserve, or attack order was found, check other roles
  ...
  if (name == undefined) {
    //count the number of reservers globally
    for (let roomName in this.memory.minReservers) {
      numberOfReservers[roomName] = _.sum(
        Game.creeps,
        c => c.memory.role == "reserver" && c.memory.target == roomName
      );

      if (numberOfReservers[roomName] < this.memory.minReservers[roomName]) {
        name = this.createReserver(roomName);
      }
    }
  }
...
// // create a new function for StructureSpawn
// StructureSpawn.prototype.createAttacker = function(target) {
//   return this.createCreep([ATTACK, MOVE], undefined, {
//     role: "attacker",
//     target: target
//   });
// };
...
//   console.log("num partrs for lorry: ", numberOfParts);
//   // make sure the creep is not too big (more than 50 parts)
//   numberOfParts = Math.min(numberOfParts, Math.floor(50 / 3));
//   var body = [];
